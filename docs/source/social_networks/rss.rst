...
Adds a **whitelist** filter to the stream. In this example, if the item contains ``first word`` and/or (see below) ``second word`` **in its title**, the item will be sent to the webhook, otherwise it will be ignored. You can set one or more words, case-insensitive.
...
Sets a new message template for when items are posted. Custom headers can have a maximum of **1024** characters.
